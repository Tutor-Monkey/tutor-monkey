...
function renderPart(part: EditorPart, index: number): ReactNode {
  if (part.kind === "mention") {
    return (
      <span
        key={`${part.doc.id}-${index}`}
        contentEditable={false}
        data-mention-id={part.doc.id}
        className="mx-0.5 inline-flex select-none items-center rounded-md bg-indigo-100 px-1.5 py-0.5 align-baseline text-sm font-medium text-indigo-700"
      >
        @{part.doc.filename}
      </span>
    );
...
export function MaterialsComposer({ currentWorkspaceId, onGenerated }: MaterialsComposerProps) {
  ...
  async function generateMaterial() {
    ...
    setGenerating(true);
    const explicitIds = selectedDocs.filter(isReadySourceDoc).map((doc) => doc.id);
    const related = docs.filter((doc) => !explicitIds.includes(doc.id) && isReadySourceDoc(doc)).slice(0, 4);
    const materialIds = Array.from(new Set([...explicitIds, ...related.map((doc) => doc.id)]));
    if (materialIds.length === 0) { setGenerating(false); return; }
    const result = await requestWorkspaceGeneration(currentWorkspaceId, {
      prompt,
      materialIds,
      confirmedMaterialIds: explicitIds,
    });
    setGenerating(false);
    if (result.ok) {
      ...
      <div className="relative">
        ...
        <div className="rounded-3xl border border-gray-300 bg-white p-3 shadow-sm transition-shadow focus-within:border-gray-500 focus-within:shadow-md">
          <div className="flex items-end gap-3">
            ...
            <button type="button" aria-label="Generate material" onClick={() => void generateMaterial()} disabled={generating || text.trim() === ""} className="mb-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gray-900 text-white transition hover:bg-gray-700 disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-400">
